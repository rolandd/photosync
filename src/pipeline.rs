...
fn file_walker(
    ...
    // Optimization: Pre-allocate OsString vector outside the loop.
    // This avoids the allocation and UTF-8 validation overhead of calling
    // `to_string_lossy()` on every directory entry, allowing direct OsStr comparisons.
    let exclude_os_dirs: Vec<std::ffi::OsString> = exclude_dirs
        .into_iter()
    ...
    let walker = WalkDir::new(&source_dir)
        ...
        .filter_entry(move |e: &walkdir::DirEntry| {
            ...
            !exclude_os_dirs.iter().any(|ex| name == ex)
        });
...
fn atomic_copy_file<R: Read>(
    ...
    match io::copy(reader, &mut writer) {
        ...
        Err(e) => {
            // Cleanup on failure to prevent corrupt artifacts (CWE-459)
            // Drop writer first: Windows requires closed handle for removal
            drop(writer);
