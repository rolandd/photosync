...
fn file_walker(
    ...
    // Performance optimization: Pre-allocate OsStrings for direct OsStr comparison
    // to avoid the allocation and UTF-8 validation overhead of calling
    // to_string_lossy() on every single directory entry.
    ...
    let walker = WalkDir::new(&source_dir)
        ...
        .filter_entry(move |e: &walkdir::DirEntry| {
            ...
            !exclude_os_dirs.iter().any(|ex| name == ex.as_os_str())
        });
