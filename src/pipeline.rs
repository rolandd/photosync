...
fn file_processor(
    ...
    for path in rx {
        ...
        // Open file once here to reuse handle downstream
        let mut file = match File::open(&path) {
            Ok(f) => f,
            Err(_) => continue,
        };

        // Inner scope to borrow file for EXIF parsing
        let (model, date) = {
            // Check if file is compatible with EXIF parsing
        ...
        if let (Some(model), Some(date)) = (model, date) {
            // Rewind file for reading in the handler
...
fn file_handler(
    ...
    for info in rx {
        ...
        // Reuse the already open file handle from processor
