...
fn file_walker(
    ...
    // Performance optimization: Pre-allocate OsString values for exclusion comparisons
    // to avoid the allocation and UTF-8 validation overhead of calling to_string_lossy()
    // on every single directory entry during the hot traversal loop.
    let exclude_os_strings: Vec<std::ffi::OsString> = exclude_dirs
        .into_iter()
