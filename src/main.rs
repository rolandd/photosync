...
fn run_text_mode(rx: std::sync::mpsc::Receiver<ProgressMsg>) -> Summary {
...
fn main() -> Result<()> {
    ...
    let (walker_tx, processor_rx) = std::sync::mpsc::sync_channel::<PathBuf>(CHANNEL_BUFFER_SIZE);
    let (processor_tx, handler_rx) = std::sync::mpsc::sync_channel::<FileInfo>(CHANNEL_BUFFER_SIZE);

    // Create progress channel for TUI
    // Note: if buffer is full, workers block.
    let (progress_tx, progress_rx) = std::sync::mpsc::sync_channel::<ProgressMsg>(CHANNEL_BUFFER_SIZE);
