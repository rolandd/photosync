...
## 2025-02-14 - Fix DoS via FIFOs in FileComparator
**Vulnerability:** The `compare_file` method in `src/pipeline.rs` opened a file path given by `path2` without verifying it was a regular file first. Because `path2` is determined based on the source filename, an attacker could plant a FIFO (named pipe) with an identical name in the destination folder. When `compare_file` calls `File::open` on the FIFO, the application would block indefinitely, leading to a Denial of Service.
**Learning:** `fs::metadata` size checks alone are insufficient for determining if an I/O operation is safe to proceed with. We must explicitly check `.is_file()` to reject blocking special files like FIFOs.
**Prevention:** Always verify `fs::metadata()?.is_file()` before calling `File::open` on user-influenced or external file system paths. When testing for this, ensure the test correctly hangs (by not spawning background readers/writers) if the bug is present, so the test suite catches it via timeouts.
