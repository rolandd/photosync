...
## 2026-06-03 - Terminal Injection via System Error Messages
**Vulnerability:** While file paths were identified as a potential source of terminal injection, system error messages (like `io::Error`) were overlooked. These error messages often wrap the underlying file paths provided by the user. If a malicious path containing ANSI escape sequences fails an OS check (e.g., during `File::open` or `metadata`), the un-sanitized path will be embedded in the error string and subsequently displayed in the terminal via `report_error`, leading to terminal manipulation.
**Learning:** System error messages that incorporate user-controlled data (like file paths) are just as dangerous as the data itself. You cannot trust the output of `Error::to_string()` or `format!("{e}")` to be terminal-safe if it wraps unsafe input.
**Prevention:** Sanitize the entire formatted error string (e.g., using `paths::sanitize_str`) before passing it to the UI or logging it to the terminal. When using generics like `impl Into<String>`, ensure the `.into()` result is assigned to a local variable before passing its reference to the sanitizer to avoid lifetime/borrowing issues.
