...
## 2026-03-09 - Incomplete/Incorrect Display Sanitization
**Vulnerability:** Filenames extracted from paths could be displayed unsanitized during error reporting (e.g. `report_error`), permitting terminal injection via valid UTF-8 control codes. Additionally, generic error strings (like standard I/O errors) were improperly passed through the path sanitization function `paths::sanitize_str`, potentially mangling valid system debugging text.
**Learning:** We must apply sanitization strictly and carefully. Only path/filename components that will be logged or displayed should be sanitized with `paths::sanitize_str`. Pre-formatted generic system errors (such as `std::io::Error`) should not be passed through path-specific sanitizers. We also must be careful of temporary value lifetimes in Rust when composing string formatting with `.into()`.
**Prevention:** Sanitize only the explicit path fields immediately before passing them to the UI/log structure. Store intermediate `.into()` string copies in variables where needed for references, but avoid blanket-sanitizing underlying generic `e.to_string()` output.
