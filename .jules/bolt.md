...
## 2026-05-18 - [Optimistic File Opening]
**Learning:** Reusing the `File` handle opened for EXIF parsing in the copy stage avoids redundant `open` syscalls. `nom-exif`'s `MediaSource::seekable` accepts `&mut File` and properly scopes the borrow, allowing the file handle to be reset (`seek(Start(0))`) and reused.
**Action:** When a file is processed in multiple stages, pass the open `File` handle (and reset its position) instead of reopening it, to save syscall overhead.
