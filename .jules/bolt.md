...
## 2026-06-20 - [File Traversal Optimization]
**Learning:** Calling `to_string_lossy()` on `OsStr` for every entry during directory traversal adds significant overhead due to string allocation and UTF-8 validation in the hot loop.
**Action:** Pre-allocate `OsString` representations of strings once before loops, and perform direct comparisons using the native `OsStr` type instead of coercing `OsStr` to `String` repeatedly.
